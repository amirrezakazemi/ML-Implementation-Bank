...
reward_model_path = '/home/ubuntu/volume/math-shepherd/'
dataset_path = "/home/ubuntu/volume/GSM8K_test.jsonl"

gpu_device = "0"

output_path = os.path.join(os.path.dirname(dataset_path), "GSM8K_test_llama_instruction_bestofn_prediction_8shot.jsonl")
...
reward_model = AutoModelForCausalLM.from_pretrained(
    reward_model_path, device_map=f"cuda:{gpu_device}"
)
...
candidate_tokens = reward_tokenizer.encode(f"{good_token} {bad_token}")[1:] # [648, 387]
step_tag_id = reward_tokenizer.encode(f"{step_tag}")[-1] # 12902
...
with open(dataset_path, 'r') as infile, open(output_path, 'w') as outfile:
    ...
    for line in tqdm(infile, total=total_lines, desc="Processing lines"):
        ...
        max_avg_score, bestofn_solution = float("-inf"), None 

        decoded_solutions = tokenizer.batch_decode(
            solutions[:, len_sample:]
        )
